...
uses
  ...
  DBGrids, ComCtrls, Gauges,TlHelp32,Excel2000, Menus, pngimage, jpeg;
...
type
  TForm1 = class(TForm)
    ...
    mniRep: TMenuItem;
    Setting: TMenuItem;
    mniSettingini: TMenuItem;
    mniSQLSettingini: TMenuItem;
...
//    function  WordReplace(str:String ; OldText:String ; NewText:String):String ;
    ...
    procedure mniRepClick(Sender: TObject);
    procedure mniSettinginiClick(Sender: TObject);
    procedure mniSQLSettinginiClick(Sender: TObject);
  type
...
uses GRADE,login,UnitMyThread,Table,Rep,SettingSet,SQLtext;
...
procedure TForm1.mniSettinginiClick(Sender: TObject);
var
Form : TSSet;
TabSheet : TTabSheet;
I: Integer;
...
begin
  ...
  for I := 0 to pgc1.PageCount - 1 do
    if  pgc1.Pages[i].Caption  = 'Setting.ini_設定' then
      begin
  ...
  if not b then
    begin
      ...
      TabSheet.Name := 'tsSetting';
      TabSheet.Caption := 'Setting.ini_設定';
      TabSheet.PageControl := pgc1;
      TabSheet.Align := alClient;
      Tablist.AddObject('Setting.ini_設定',TObject(TabSheet));
...
procedure TForm1.mniSQLSettinginiClick(Sender: TObject);
var
Form : TSQLSetting;
TabSheet : TTabSheet;
I: Integer;
...
begin
  ...
  try
  for I := 0 to pgc1.PageCount - 1 do
    if  pgc1.Pages[i].Caption  = 'SQLSetting.ini_設定' then
      begin
        pgc1.ActivePageIndex := i;
        b:=true;
      end;
  if not b then
    begin
      with pgc1 do
      TabSheet := TTabSheet.Create(self);
      TabSheet.Name := 'tsSQLSetting';
      TabSheet.Caption := 'SQLSetting.ini_設定';
      TabSheet.PageControl := pgc1;
      TabSheet.Align := alClient;
      Tablist.AddObject('SQLSetting.ini_設定',TObject(TabSheet));
      pgc1.ActivePageIndex := pgc1.PageCount-1;
    end
  except
    FreeAndNil(TabSheet);
...
procedure TForm1.mniRepClick(Sender: TObject);
var
Form : TRepExport;
TabSheet : TTabSheet;
I: Integer;
...
begin
  ...
  try
    for I := 0 to pgc1.PageCount - 1 do
      if  pgc1.Pages[i].Caption  = '名冊套印' then
      begin
          pgc1.ActivePageIndex := i;
          b:=true;
      end;
    ...
    TabSheet := TTabSheet.Create(self);
    TabSheet.Name := 'tsRep';
    TabSheet.Caption := '名冊套印';
    TabSheet.PageControl := pgc1;
    TabSheet.Align := alClient;
    Tablist.AddObject('名冊套印',TObject(TabSheet));
    pgc1.ActivePageIndex := pgc1.PageCount-1;
    end
  except
    FreeAndNil(TabSheet);
