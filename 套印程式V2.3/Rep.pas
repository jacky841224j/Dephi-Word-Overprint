    ...
    begin
      ...
      for i:= 1 to eof  do
       begin
          ...
          if ( i = 1 ) or  (namechk = 1 ) then
            begin
              if cbbTitle4.ItemIndex <> -1  then
                begin
                  name4 := FieldByName(cbbTitle4.Items[cbbTitle4.ItemIndex]).AsString;
                  name3 := FieldByName(cbbTitle3.Items[cbbTitle3.ItemIndex]).AsString;
                  name2 := FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString;
                end
              else if cbbTitle3.ItemIndex <> -1 then
                begin
                  name3 := FieldByName(cbbTitle3.Items[cbbTitle3.ItemIndex]).AsString;
                ...
                name2 := FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString;
               name1  := FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString;
               namechk:= 0;
            end;
         ...
         if chk = 0 then
          ...
          if (chk <> 0) or ( i = RecordCount) then
            begin
              //套印資料
              ...
              if cbbTitle4.ItemIndex <> -1 then
                begin
                  if chkWord.Checked  then  WordDoc.SaveAs(SavePath + name1 +'-'+name2+'-'+name3+'-'+name4 +'.docx');
                  WordDoc.ExportAsFixedFormat(SavePath + name1 +'-'+name2 +'-'+name3+'-'+name4 +'.pdf',17);
                end
              else if cbbTitle3.ItemIndex <> -1 then
                begin
                  if chkWord.Checked  then  WordDoc.SaveAs(SavePath + name1 +'-'+name2 +'-'+name3+'.docx');
                  WordDoc.ExportAsFixedFormat(SavePath + name1 +'-'+name2 +'-'+name3+ '.pdf',17);
                end
              else if cbbTitle2.ItemIndex <> -1 then
                begin
                  if chkWord.Checked  then  WordDoc.SaveAs(SavePath + name1 +'-'+name2 +'.docx');
                  WordDoc.ExportAsFixedFormat(SavePath + name1 +'-'+name2+ '.pdf',17);
                end
              else if cbbTitle1.ItemIndex <> -1  then
                begin
                  if chkWord.Checked  then  WordDoc.SaveAs(SavePath + name1 +'.docx');
                  WordDoc.ExportAsFixedFormat(SavePath + name1 + '.pdf',17);
                end ;
...
begin
  ...
  strngrdCheckList.Cells[4,0] := #32#32#32#32'---【輸入條件】---' ;
