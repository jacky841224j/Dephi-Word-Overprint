...
uses
  ...
  GIFImg, Buttons, jpeg,shellapi, pngimage;
...
var
  S_Photo,WordFileName,find,LogTemp: string;
...
begin
  ...
  if chkPic.Checked then
    begin
      S_Photo := edtPic.Text + FieldByName(myinifile.readstring('Photo','pic','')).AsString ;
      WordDoc.Tables.Item(1).Cell(StrToInt(edtCols.Text),StrToInt(edtRows.Text)).range.InlineShapes.AddPicture(S_Photo,false,true);
    end;
  end;
   {$ENDREGION}
end;
...
begin
  ...
  temp1:= POS('@', temp);
  strtemp:= copy(temp, 0,temp1-1);       //擷取db 最大長度字串
  ...
  temp1:= POS('@', temp);
  temp2:= POS('%', temp);
  strtemp:= copy(temp, temp1+1,temp2-1);       //擷取db 最大長度字串
  for i:=1 to Length(strtemp) do            //去除多餘字串
