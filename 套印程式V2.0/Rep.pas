...
//  end;
  ...
  TRepExport = class(TForm)
    ...
    procedure chklstTitleClick(Sender: TObject);
...
var
  ...
  timeh : longint = 0 ;
  timem : longint = 0 ;
  SqlFS,SqlPV,SqlEP,SqlRep : string ;
implementation
...
var
  WordFileName,LogTemp: string;
  SavePath,chkstr,chkstr2,chkstr3 : string;
  WordApp, WordDoc, myRange, vSaveNone : Variant;
  i,p,x,y,count,row,total,chk: Integer;
  sqlstr : TStringList;
  SaveTxt:TextFile;
begin
 y:= 1;
...
procedure TRepExport.chklstTitleClick(Sender: TObject);
var
i : Integer;
begin
lstchk.Clear;
...
var
  WordFileName,LogTemp: string;
  chkstr,chkstr2,chkstr3 : string;
  WordApp, WordDoc, myRange : Variant;
  i,p,x,y,row,total: Integer;
  chk : Boolean;
begin
  form1.KillExcelTask;
  y:= 1;
...
begin
  ...
  strtemp:= copy(temp, temp1+1,temp2-1);       //擷取字串
  for i:=1 to Length(strtemp) do            //去除多餘字串
  if not (strtemp[i] = '$') then SqlRep := SqlRep + strtemp[i]
  else break;
  strtemp := '';
  {$ENDREGION}
