 ...
 ShowMessage('資料內含個資，請妥善保存！');
  ...
  try
    ...
    begin
      ...
      while not Eof do
       begin
        if chkExit.Checked then    //判斷是否離開
          begin
        ...
        if cbbTitle4.ItemIndex <> -1 then
          begin
            //判斷欄位是否相同
            if (Eof) or (( chkstr2 <> qryExport.FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString ) or ( chkstr <> FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString ) or ( chkstr3 <> FieldByName(cbbTitle3.Items[cbbTitle3.ItemIndex]).AsString )or ( chkstr4 <> FieldByName(cbbTitle4.Items[cbbTitle4.ItemIndex]).AsString ) )
              and ( i > 1 ) then chk := 4
            else if total <> 0 then  WordDoc.Tables.Item(y).Range.Rows.Add(EmptyParam);
            //儲存檔名
            if  total = 0 then
              nametemp := FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString + '-' +  FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString  + '-' + FieldByName(cbbTitle3.Items[cbbTitle3.ItemIndex]).AsString + '-' + FieldByName(cbbTitle4.Items[cbbTitle4.ItemIndex]).AsString;
            //儲存當前欄位，判斷下一筆欄位是否相同
          ...
          begin
            //判斷欄位是否相同
            if (Eof) or (( chkstr2 <> qryExport.FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString ) or ( chkstr <> FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString ) or ( chkstr3 <> FieldByName(cbbTitle3.Items[cbbTitle3.ItemIndex]).AsString ) )
              and ( i > 1 )   then chk := 3
            else if total <> 0 then  WordDoc.Tables.Item(y).Range.Rows.Add(EmptyParam);
            //儲存檔名
            if  total = 0 then
              nametemp := FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString + '-' +  FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString  + '-' + FieldByName(cbbTitle3.Items[cbbTitle3.ItemIndex]).AsString ;
            //儲存當前欄位，判斷下一筆欄位是否相同
            chkstr3 := FieldByName(cbbTitle3.Items[cbbTitle3.ItemIndex]).AsString;
            chkstr2 := FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString;
            chkstr  := FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString;
          ...
          begin
            //判斷欄位是否相同
            if (Eof) or (( chkstr2 <> qryExport.FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString ) or (( chkstr <> FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString )) ) and ( i > 1 ) then
              chk := 2
            else if total <> 0 then  WordDoc.Tables.Item(y).Range.Rows.Add(EmptyParam);
            //儲存檔名
            if  total = 0 then
              nametemp := FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString + '-' +  FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString ;
            //儲存當前欄位，判斷下一筆欄位是否相同
            chkstr2 := FieldByName(cbbTitle2.Items[cbbTitle2.ItemIndex]).AsString;
            chkstr  := FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString;
          end
          ...
          begin
            ...
            //儲存檔名
            if  total = 0 then
              nametemp := FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString;
            //儲存當前欄位，判斷下一筆欄位是否相同
            chkstr  := FieldByName(cbbTitle1.Items[cbbTitle1.ItemIndex]).AsString;
          end;

        //判斷是否為第一筆資料，是的話則取代欄位
        if  (i = 1) or (total = 0) then
        ...
        else if (chk <> 0) or ( i = RecordCount) then
          begin
            //套印總筆數
            myRange.Find.Execute(FindText:=edtTotal.Text,ReplaceWith:=total , Replace:=2);
            //存檔
            if chkWord.Checked  then  WordDoc.SaveAs(SavePath + nametemp +'.docx');
            WordDoc.ExportAsFixedFormat(SavePath + nametemp + '.pdf',17);
  ...
  for p := 1 to strngrdCheckList.RowCount  do     //儲存有搜尋條件的欄位
    ...
    if chk = 0 then qryExport.SQL.Text := SqlEP + orderby
      ...
      begin
        ...
        strtemp := '';
        qryExport.SQL.Text := SqlEP + SQLExport +', where 1=1 '+QuotedStr(find)+orderby;
      end;
...
procedure TRepExport.strngrdCheckListSetEditText(Sender: TObject; ACol,
...
begin
  ...
  strngrdCheckList.Cells[2,0] := '---【取代文字】---' ;
  strngrdCheckList.Cells[3,0] := '=' ;
  strngrdCheckList.Cells[4,0] := '---【輸入條件】---' ;
